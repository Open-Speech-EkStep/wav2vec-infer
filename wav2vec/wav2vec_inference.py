...
def parse_transcription(model_path, dict_path, wav_path, cuda):
    result = ''
    if cuda:
        gpu_model = load_gpu_model(model_path)
        result = get_results( wav_path , dict_path,cuda,model=gpu_model)
    else:
        cpu_model = load_cpu_model(model_path)
        result = get_results( wav_path , dict_path,cuda,model=cpu_model)
